...
> Queue-based (`/runsync`) endpoints require the Runpod serverless envelope `{"input": {"<function-parameter-name>": ...}}`. Here the worker is `cpu_hello(input_data)`, so the inner key is `input_data`.

```bash
curl -X POST http://localhost:8888/cpu_worker/runsync \
  -H "Content-Type: application/json" \
  -d '{"input": {"input_data": {"name": "Flash User"}}}'
```

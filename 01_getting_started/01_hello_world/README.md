...
> Queue-based (`/runsync`) endpoints require the Runpod serverless envelope `{"input": {"<function-parameter-name>": ...}}`. Here the worker is `gpu_hello(input_data)`, so the inner key is `input_data`.
